...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Color the logger name (limira_agent)
...
def bootstrap_logger() -> logging.Logger:
    """Configure the limira_agent logger with consistent formatting"""

    global logger

    # Configure limira_agent logger
    limira_agent_logger = logging.getLogger("limira_agent")

    # Check if logger already has handlers to prevent duplicate configuration
    if limira_agent_logger.handlers:
        logger = limira_agent_logger
        return limira_agent_logger
    ...
    limira_agent_logger.addHandler(handler)
    limira_agent_logger.setLevel(logging.DEBUG)

    # Disable propagation to prevent duplicate logging from root logger
    limira_agent_logger.propagate = False
...
class TaskLog:
    ...
    def log_step(
        ...
        if "Tool Call Start" in step_name:
            ...
            icon = "🐍 "
        elif "search_and_scrape_webpage" in step_name.lower():
            icon = "🔍 "
