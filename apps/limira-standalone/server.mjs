...
server.listen(port, host, () => {
	console.log(`limira standalone frontend listening on http://${host}:${port}`);
	console.log(`proxying /api/limira/* to ${backendUrl}`);
});

function isAllowedApiPath(pathname) {
	return isLimiraApiPath(pathname);
}

function isLimiraApiPath(pathname) {
	return pathname === '/api/limira' || pathname.startsWith('/api/limira/');
}
