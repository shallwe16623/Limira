<script lang="ts">
	...
	const DEFAULT_PINNED_ITEMS = ['limra', 'notes', 'workspace'];
	...
	$: pinnedItems = ensureLimraPinned($settings?.pinnedMenuItems ?? DEFAULT_PINNED_ITEMS);

	const ensureLimraPinned = (items) => (items.includes('limra') ? items : ['limra', ...items]);
	...
	const getMenuItemMeta = (id) => {
		const items = {
			limra: { label: 'limra', href: '/limra', iconType: 'limra' },
