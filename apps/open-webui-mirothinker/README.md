...
- `DOWNLOAD_BASE_URL`: optional browser-reachable trusted proxy base URL for archive links. If blank, `RUNNER_BASE_URL` is used for local/dev deployments.
...
Do not expose `RUNNER_SERVICE_TOKEN` to browser JavaScript. For production downloads, put Runner API behind an Open WebUI backend route or reverse proxy that injects the trusted headers server-side and applies the same authenticated Open WebUI user context.
