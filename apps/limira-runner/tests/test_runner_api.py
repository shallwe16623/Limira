...
def test_runner_api_startup_schedules_resumable_langgraph_worker_without_sse(
    tmp_path,
):
    store = TaskStore(tmp_path / "tasks.sqlite3")
    blocking_stream = BlockingStream()
    record = seed_running_task(
        store,
        task_id="task-resumable-langgraph-startup",
        worker_id="worker-resume",
        lease_expires_at="2026-06-06T12:05:00+00:00",
    )
    checkpoint = _checkpoint_envelope(
    ...
    queued = store.get_task(record.task_id)
    assert queued.status == "queued"
    assert app[TASK_WORKERS_KEY] == {}
    assert store.get_task_checkpoint(record.task_id)["status"] == "queued"

    async def run_startup_without_sse():
        client = TestClient(TestServer(app))
        try:
            await client.start_server()
            await asyncio.wait_for(blocking_stream.started.wait(), timeout=1.0)
            ...
            assert persisted_checkpoint["current_node"] == "compress"
            assert persisted_checkpoint["source_ledger"] == checkpoint["source_ledger"]
            assert persisted_checkpoint["evidence_ledger"] == checkpoint[
                "evidence_ledger"
            ]
            assert persisted_checkpoint["executor_state"]["resume_status"] == "running"
        finally:
    ...
    asyncio.run(run_startup_without_sse())
